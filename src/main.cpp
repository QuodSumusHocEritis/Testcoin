...
uint256 hashGenesisBlock("0x1534b34660e450deda07d8aaa772483cb3795ccd3ebb3322306dd19735fccf4c");
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 10000 blocks, which will occur approximately every 7 days
    nSubsidy >>= (nHeight / 10000); // Testcoin: 10K blocks ~7 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Testcoin: diff calc every 2 days
static const int64 nTargetSpacing = 1 * 60; // Testcoin: 1 minute block time
...
bool LoadBlockIndex()
{
    if (fTestNet) //These are the test net peer magic bytes. Leave the first digits, but change the second.  Obviously the 0x indicates the radix.
    {
        pchMessageStart[0] = 0xf1;
        pchMessageStart[1] = 0xc2;
        pchMessageStart[2] = 0xb3;
        pchMessageStart[3] = 0xd4;
        hashGenesisBlock = uint256("0xc01e1ebc92d71d4a950766d340f534f9f6d7e78679beef204a1d198c367b1d51");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN 27/Sep/2022 Japan holds national funeral for Shiro Abe";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042b447306c4bf02394e0243a16bcca46dd8dd321ae37b1f3ac5d4d4c2a91166cb6966bb8f970e874b505753e19474470f3e5575fed22d6692469cf24039f149bd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1664324178;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084755741;

        if (fTestNet)
        {
            block.nTime    = 1664324160;
            block.nNonce   = 385346089;
        }

	//Genesis block code | Set first proposition to true if you need to generate hashes for test and main net genesis blocks.  Otherwise, false or remove if-block altogether
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x84cd7e0cff89374801529d1853b81973392f637a2191ee773dde9eb18cf20f01"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xcf, 0xba, 0xd0 }; // Testcoin: increase each by adding 2 to bitcoin's value.
